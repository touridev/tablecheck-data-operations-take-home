...
def get_business_answers():
    ...
    customers = conn.execute('SELECT COUNT(DISTINCT customer_name) FROM restaurant_transactions WHERE restaurant_name = "the-restaurant-at-the-end-of-the-universe"').fetchone()[0]
    ...
    revenue = conn.execute('SELECT SUM(food_cost) FROM restaurant_transactions WHERE restaurant_name = "the-restaurant-at-the-end-of-the-universe"').fetchone()[0]
    ...
    cur = conn.execute('''
        ...
        ORDER BY restaurant_name
    ''')
    profitable_dishes = cur.fetchall()
